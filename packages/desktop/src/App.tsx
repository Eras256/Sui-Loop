...
import { AlertOctagon, RefreshCw, Wifi, WifiOff } from "lucide-react";
...
function App() {
    ...
    const [logs, setLogs] = useState<LogEntry[]>([
        { ts: "--:--:--", msg: "Connecting to agent...", type: "info" }
    ]);
    const wsRef = useRef<WebSocket | null>(null);

    const addLog = (msg: string, type: LogEntry["type"] = "info") => {
        const ts = new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" });
        setLogs(prev => [...prev.slice(-20), { ts, msg, type }]);
    };
    ...
    useEffect(() => {
        ...
        const connect = () => {
            try {
                ...
                ws.onerror = () => addLog("WS connection error — retrying in 10s", "error");
    ...
    return (
        <div className="w-[300px] h-[420px] bg-black/90 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden flex flex-col relative text-white">
            ...
            <div className="h-12 border-b border-white/10 bg-white/5 flex items-center justify-between px-4" data-tauri-drag-region>
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-white/30 font-mono">v0.0.7</span>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="px-3 pt-3">
                <div className="bg-white/5 border border-white/5 rounded-lg p-2.5">
                    <div className="flex justify-between items-center mb-1.5">
                        <span className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Agent</span>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${agentStatus === "ACTIVE" ? "bg-green-500/20 text-green-400" :
            ...
            <div className="flex-1 mx-3 mt-2 mb-2 bg-black/50 rounded-lg p-2 font-mono text-[9px] overflow-y-auto border border-white/5 flex flex-col gap-0.5">
                {logs.map((log, i) => (
                    <div key={i} className={`truncate ${logColor(log.type)}`}>
                        <span className="text-white/20 mr-1">[{log.ts}]</span>{log.msg}
                    </div>
                ))}
            </div>
            ...
            <div className="px-3 pb-3">
                <button
                    ...
                    className={`w-full py-2.5 rounded-lg font-bold text-xs flex items-center justify-center gap-2 transition-all disabled:opacity-40 disabled:cursor-not-allowed
                        ${agentStatus === "ACTIVE"
