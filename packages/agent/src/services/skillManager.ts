...
export interface SkillContext {
    ...
    log: (level: 'debug' | 'info' | 'warn' | 'error' | 'success', message: string) => void;
}
...
export class SkillManager extends EventEmitter {
    ...
    async loadSkill(skillPath: string): Promise<SkillInstance | null> {
        ...
        console.log(`✅ Loaded skill: ${manifest.name} v${manifest.version} from ${skillPath}`);
    ...
    private async installFromLoopHub(slug: string): Promise<string> {
        ...
        const { getLoopHubClient } = await import('./loopHub.js');
        const loopHub = getLoopHubClient();

        console.log(`📦 Installing skill from loophub:${slug}`);

        // Get skill details from LoopHub
        let skill: any = loopHub ? await loopHub.getSkill(slug) : null;

        // Fallback metadata if not found (should not happen if called via API)
        if (!skill) {
            console.warn(`Skill ${slug} not found in LoopHub, using fallback metadata`);
            skill = {
                id: slug,
                slug: slug,
...
permissions:
${(skill.permissions || []).map((p: string) => `  - ${p}`).join('\n')}
actions:
${(skill.actions || []).map((a: any) => `  - name: ${a.name}
    description: "${a.description}"
    handler: ${a.handler}`).join('\n')}
---
...
${(skill.actions || []).length > 0 ? 'This skill provides the following actions:' : ''}

${(skill.actions || []).map((a: any) => `### ${a.name}
...
`;
        ...
        // Generate Mock Handlers for Actions
        if (skill.actions) {
 ...
 */
export default async function ${action.handler}(params, context) {
    context.log('info', 'Executing ${action.name} with params: ' + JSON.stringify(params));
    
    // Simulate processing time
    await new Promise(resolve => setTimeout(resolve, 1000));
    
    context.log('success', '${action.name} completed successfully');
...
`;
                await fs.writeFile(path.join(targetPath, `${action.handler}.js`), handlerCode);
            }
        }

        // Generate Mock Config
        if (skill.configSchema) {
            await fs.writeFile(path.join(targetPath, 'config.json'), JSON.stringify({}, null, 2));
        }
    ...
    async executeAction(
        ...
        const fullContext: SkillContext = {
            ...
            log: (level, message) => {
                ...
                consoleMethod(`[${skill.manifest.name}] ${message}`);
                this.emit('log', { skill: skill.manifest.name, level, message });
            }
        };
